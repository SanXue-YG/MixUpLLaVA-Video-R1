...
- **对照基线**：选择器默认 **M1 = B + CPPO**（项目自研质量向 + 原定效率目标；正式训可经 Phase 6 总控制台按需启动）  
- **默认保留**：加开其它模块时 **B 与 CPPO 除非显式 `false` 否则保持开启**  
- **当前主交付**：Phase 3 **模块库** + Phase 4 **选择器** + Phase 6 **总控制台**（流程优先；开训可延后）  
- **本文角色**：给出若干**推荐/备选预设**（如曾议的 B+CPPO+NGRPO+GFPO、M5q 等），供选择器 / 总控制台一键加载；**不代替**用户最终选哪套。  
- **用途**：目标明确后按决策树选用预设；经总控制台先出基线报告，再训所选方案并对比。
...
2. **M1（B+CPPO）为组内默认基线**：B 负责质量向，CPPO 负责效率；再叠模块时，不要默认「模块越多越准」。  
...
4. **CPPO / GFPO** 按报告与论文主叙事是 **加速 / 简洁**；CPPO 已纳入默认栈作效率底盘，不宜再写成准确率卖点。  
5. 因此：**M5 相对 M1 的「准」预期应主要归功于 NGRPO（及可选 MO-GRPO）**；GFPO 管简洁。若 Phase 5 显示 M5 准不动，优先试 **MO-GRPO**，而不是再加 GFPO。
...
- **相对 M1（B+CPPO）**：默认栈无负信号校准；难样本上全错 group 仍可能白更新 → **再叠 NGRPO 后相对 M1 的「准」预期应主要归功于 NGRPO，而非再强调 CPPO**。
...
|----|------|----------------------|------|----------|
| **M5**（主路径） | B + CPPO + NGRPO + GFPO | 准靠 **B+NGRPO**；速度 CPPO；简洁 GFPO | 高（对齐 Phase1 C2 量级预期） | 总控制台推荐预设之一 |
...
| `schedule.md` Phase 4 | **选择器能力**；本文预设 = 可选一键组合；完整开训可延后 |
| `schedule.md` Phase 5 | **评测流水线**；基线 vs 当次方案（不必固定评旧「M5」） |
| `schedule.md` Phase 6 | **总控制台**：调参 → 基线报告 → 优化对比报告 |
...
*维护：随 Phase 3/4/6 实现进度更新「是否已实现」列；总控制台实跑出数后在本文末追加实测结论。*
