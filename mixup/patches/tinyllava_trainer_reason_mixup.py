...
def _load_mixup_config(args):
    """Load MixUpConfig from args / sidecar JSON / env."""
    import sys
    from pathlib import Path

    # Allow Drive MixUp repo on sys.path via sidecar written by apply_mixup_to_repo
    here = Path(__file__).resolve().parent
    pth = here / "_mixup_path.pth"
    if pth.is_file():
        root = pth.read_text(encoding="utf-8").strip()
        if root and root not in sys.path:
            sys.path.insert(0, root)

    try:
        from mixup.config import MixUpConfig
        from mixup.trainer_mixup import load_mixup_config_sidecar
    except ImportError:
        return None

    side = load_mixup_config_sidecar(here)
    if side is not None:
        return side

    raw = getattr(args, "mixup_config", None)
    if isinstance(raw, dict):
        return MixUpConfig.from_dict(raw, strict=False)
    # Fallback: map legacy cppo_pruning_rate
    rate = float(getattr(args, "cppo_pruning_rate", 0.0) or 0.0)
    return MixUpConfig(cppo=rate > 0, cppo_pruning_rate=rate)
...
class LLaVATrainer_Reason(Trainer):
    def __init__(
        ...
        self.mixup_cfg = _load_mixup_config(args)
        self.beta = 0.01 #args.beta
        if self.mixup_cfg is not None and self.mixup_cfg.project_baseline:
            self.beta = float(self.mixup_cfg.baseline_beta)
        self.num_generations = 4 # = G in the GRPO paper
        self.num_frame = 4
        # CPPO: Completion Pruning (0 = off/GRPO, e.g. 0.5 = prune 50% lowest |advantage|)
        if self.mixup_cfg is not None and self.mixup_cfg.cppo:
            self.cppo_pruning_rate = float(self.mixup_cfg.cppo_pruning_rate)
        else:
            self.cppo_pruning_rate = getattr(args, "cppo_pruning_rate", 0.0)
    ...
    def compute_loss(self, model, inputs, num_items_in_batch=None):
        ...
        # MixUp pipeline: rewards → advantages → optional CPPO prune / GMPO flag
        if self.mixup_cfg is not None:
            from mixup.trainer_mixup import run_advantage_pipeline
            from mixup.gmpo import geometric_mean_loss
            ...
            combined_ids = ctx["combined_ids"]
            prompt_inputs = ctx["prompt_inputs"]
            rewards_per_func = ctx.get("rewards_per_func", rewards_per_func)
            if ctx.get("preferred_beta") is not None:
                self.beta = float(ctx["preferred_beta"])
            use_gmpo = bool(ctx.get("use_gmpo_loss"))
            gmpo_eps = float(ctx.get("gmpo_eps", 0.2))
        else:
            ...
            advantages = advantages + torch.randn_like(advantages) * 0.02
            if getattr(self, "cppo_pruning_rate", 0.0) > 0 and advantages.numel() > 1:
                k_prune = max(0, int(advantages.numel() * self.cppo_pruning_rate))
                k_prune = min(k_prune, advantages.numel() - 1)
                if k_prune > 0:
                    abs_adv = torch.abs(advantages)
                    _, idx_smallest = torch.topk(abs_adv, k_prune, largest=False)
                    keep_mask = torch.ones(advantages.shape[0], dtype=torch.bool, device=advantages.device)
        ...
        # logging std: use pre-prune rewards length safely
        try:
            g = min(self.num_generations, max(1, int(rewards.numel())))
            if rewards.numel() % g == 0:
                std_grouped_rewards = rewards.view(-1, g).std(dim=1)
            else:
                std_grouped_rewards = rewards.std().unsqueeze(0)
        except Exception:
            std_grouped_rewards = rewards.new_zeros(1)

        # Forward only on (possibly pruned) batch for CPPO speedup
        ...
        if use_gmpo:
            ...
            loss = geometric_mean_loss(
                per_token_logps,
                None,  # no π_old in this trainer; on-policy fallback
                advantages,
                completion_mask,
                torch.tensor(self.beta, device=advantages.device),
                per_token_kl,
                eps=gmpo_eps,
            )
        else:
        ...
        if self.mixup_cfg is not None:
            self._metrics["mixup"].append(float(len(self.mixup_cfg.enabled())))
